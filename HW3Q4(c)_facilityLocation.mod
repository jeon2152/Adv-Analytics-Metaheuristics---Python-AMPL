...
var AD >= 0;			#average distance


#OBJECTIVE -----------------------------------------------

minimize distance: AD;
...
subject to average_distance:		AD = sum {i in I, j in J} d[i,j]*x[i,j]/45;	#definition of average distance AD
...
#display AD, D, sum {j in J} (c[j]*s[j] + f[j]*y[j]);
#display x, y;
